...
class EncoderDecoder(LightningModule):
    ...
    def validation_epoch_end(self, outputs):
        ...
        if self.track_metric is not None:
            assert self.track_metric in metrics
            metric_val = metrics[self.track_metric]
            if self.best_metric_val is None or self.comparator(metric_val, self.best_metric_val):
    ...
    def load_model(self):
        if self.config.load_weight != "":
            print(f"loading model from {self.config.load_weight}")
    ...
    def save_model(self, finish=False, best=False):
        if self.config.save_model and (finish or self._last_global_step_saved != self.global_step):
            fname = "finish.pt" if finish else f"global_step{self.global_step}.pt"
            fname = "best.pt" if best else fname
            model_fname = os.path.join(self.config.exp_dir, fname)
